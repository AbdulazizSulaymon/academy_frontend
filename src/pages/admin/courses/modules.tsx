...
import { useModules, modulesQueryKey } from '@/queries/models/module';
...
const ModulesPage: NextPageWithLayout = observer(function Home() {
  ...
  const { mutate: remove, isPending: isLoadingRemove } = useMutation({
    mutationKey: [modulesQueryKey],
    mutationFn: (data: Record<string, any>) => api.apis.Module.deleteOne({ where: { id: data.id } }),
    onSuccess: () => {
...
const ItemModal = observer(() => {
  ...
  const {
    ...
    name: 'modules',
    model: api.apis.Module,
    form: form,
    getOne: () =>
      api.apis.Module.findOne({
        where: { id: query.id },
        include: { course: true },
      }),
  });
  ...
  return (
    <MyDrawer
      title={`${query.add ? t('Add') || 'Add' : t('Edit') || 'Edit'} ${query.add ? t('Module') || 'Module' : ''}`}
...
ModulesPage.getLayout = function getLayout(page: ReactElement) {
  return (
    <DynamicProviders>
      <AdminLayout title="Modules">{page}</AdminLayout>
    </DynamicProviders>
  );
};
