...
import { useShopCategories, shopCategoriesQueryKey } from '@/queries/models/shop-category';
...
const ShopCategoriesPage: NextPageWithLayout = observer(function Home() {
  ...
  const { data, isLoading, isError } = useShopCategories({ ...tableFetchProps }, { enabled: !!tableFetchProps.take });
  ...
  const { mutate: remove, isPending: isLoadingRemove } = useMutation({
    mutationKey: [shopCategoriesQueryKey],
    mutationFn: (data: Record<string, any>) => api.apis.ShopCategory.deleteOne({ where: { id: data.id } }),
    onSuccess: () => {
      notifySuccess('Successfully deleted!');
      queryClient.invalidateQueries({ queryKey: [shopCategoriesQueryKey] });
    },
  });
  ...
  return (
    <Box>
      ...
      <Table
        ...
        queryKey={[shopCategoriesQueryKey]}
...
const ItemModal = observer(() => {
  ...
  const {
    ...
    name: 'shop-categories',
    model: api.apis.ShopCategory,
    form: form,
    getOne: () =>
      api.apis.ShopCategory.findOne({
        where: { id: query.id },
      }),
  });
  ...
  return (
    <MyDrawer
      title={`${query.add ? t('Add') || 'Add' : t('Edit') || 'Edit'} ${
        query.add ? t('Shop Category') || 'Shop Category' : ''
      }`}
...
ShopCategoriesPage.getLayout = function getLayout(page: ReactElement) {
  return (
    <DynamicProviders>
      <AdminLayout title="Shop Categories">{page}</AdminLayout>
    </DynamicProviders>
  );
};
